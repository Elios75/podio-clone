// Local calling helper for the items_api dispatcher (items-depth slice).
// Mirrors the apiCall idiom in src/lib/api-auth.ts but targets podio.items_api
// instead of podio.api_request.
import { createClient } from "@supabase/supabase-js";
import { NextResponse } from "next/server";
import { hashKey } from "@/lib/api-auth";

function anonClient() {
  return createClient(
    process.env.NEXT_PUBLIC_SUPABASE_URL!,
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
    { db: { schema: "podio" }, auth: { persistSession: false } }
  );
}

export async function itemsApiCall(
  req: Request,
  action: string,
  params: Record<string, any> = {}
) {
  const auth = req.headers.get("authorization") ?? "";
  const raw = auth.replace(/^Bearer\s+/i, "").trim();
  if (!raw) {
    return NextResponse.json(
      { error: "Missing Authorization: Bearer <api_key>" },
      { status: 401 }
    );
  }

  const sb = anonClient();
  const { data, error } = await sb.rpc("items_api", {
    p_key_hash: hashKey(raw),
    p_action: action,
    p_params: params,
  });

  if (error) {
    const msg = error.message ?? "request failed";
    const status = msg.includes("invalid api key") ? 401
      : msg.includes("lacks write scope") ? 403
      : msg.includes("rate limit exceeded") ? 429
      : msg.includes("not found") ? 404
      : 400;
    return NextResponse.json({ error: msg }, { status });
  }
  return NextResponse.json(data);
}
