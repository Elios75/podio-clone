...
//
// PORTED COPY — source of truth is scripts/podio/mapper.mjs — keep in sync.
// Only changes from the original: TypeScript `any` annotations on exported /
// helper function signatures (the logic is byte-for-byte the same).
//
...
function podioDateToIso(s: any): string | null {
...
const PODIO_ICON_IDS: Record<number, string> = {
...
function pickIcon(name: any, podioIconId: any): string {
...
const PODIO_COLOR_MAP: Record<string, string> = {
...
function mapCategoryColor(podioColor: any, index: number): string {
...
const PODIO_TO_OUR_TYPE: Record<string, string> = {
...
export function mapApp(podioApp: any): { app: any; fields: any[]; notes: string[] } {
  ...
  const active = rawFields.filter((f: any) => f.status === "active");
  const skipped = rawFields.length - active.length;
  if (skipped > 0) {
    notes.push(`${name}: skipped ${skipped} non-active (deleted) Podio field(s)`);
  }
  active.sort((a: any, b: any) => (a.config?.delta ?? 0) - (b.config?.delta ?? 0));
  ...
  for (const pf of active) {
    ...
    switch (pf.type) {
      ...
      case "category": {
        ...
        const activeOpts = options.filter((o: any) => o.status === "active");
        ...
        field.config = {
          options: activeOpts.map((o: any, i: number) => ({
            id: String(o.id),
            label: o.text,
      ...
      case "app": {
        ...
        const appIds = refs
          .map((r: any) => r?.app_id ?? r?.app?.app_id)
...
export function mapItemValues(podioItem: any, mappedFields: any[], ctx: any = {}) {
  const memberMap: Map<any, any> = ctx.memberMap instanceof Map ? ctx.memberMap : new Map();
  const notes: string[] = [];
  ...
  for (const pf of podioItem?.fields ?? []) {
    ...
    switch (pf.type) {
      ...
      case "date": {
        ...
        if (!start) break;
        const out: any = { start: podioDateToIso(start) };
      ...
      case "email": {
        const entries = vals
          .map((v: any) => (typeof v?.value === "string" ? v.value : v?.value?.value))
          .filter(Boolean);
        if (entries.length > 0) {
...
export function mapComment(podioComment: any, ctx: any = {}) {
  const memberMap: Map<any, any> = ctx.memberMap instanceof Map ? ctx.memberMap : new Map();
...
export function mapTask(podioTask: any, ctx: any = {}) {
  const memberMap: Map<any, any> = ctx.memberMap instanceof Map ? ctx.memberMap : new Map();
